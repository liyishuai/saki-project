...
declare class SceneObject {
    ...
    /**
     * 所在场景的sceneObject列表的位置，也可以看作唯一ID，编辑器中场景对象ID即该属性
     * 默认值=0
     */
    index: number;
    /**
    ...
    y: number;
    /**
     * 显示层 0-最底层 1-中间层 2-最高层 对象所在场景的层次可参考[ClientScene]
     * 默认值=1
     */
    layerLevel: number;
    //------------------------------------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------------------------------------
    /**
     * 拥有控制权的玩家（所属玩家），非玩家的对象该值为0 主要用于判断用 
     * 默认值=0
     */
    playerUID: number;
    /**
